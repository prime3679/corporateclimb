...
// ViewBox 128x160 for tall character proportions (SVG).
// PNG character sprites imported alongside SVGs.
...
// PNG sprite overrides — these take priority over SVG entries
...
// Build the sprite URL map (SVGs first, then PNG overrides)
export function buildSpriteUrls(): Record<string, string> {
  ...
  for (const [id, url] of Object.entries(PNG_SPRITES)) {
    urls[id] = url;
  }
  return urls;
}
