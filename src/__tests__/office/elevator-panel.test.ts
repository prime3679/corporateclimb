...
describe('elevator panel (floors 1–5)', () => {
  ...
  it('maps every locked 3+ row to the employee-badge deny, not a tighter key', () => {
    const deny = {
      flag: 'flag_reader_denied_f2',
      poiId: 'poi_elevator_door_f2',
    }
    expect(elevatorDenyFor('floor_03')).toEqual(deny)
    expect(elevatorDenyFor('floor_04')).toEqual(deny)
    expect(elevatorDenyFor('floor_05')).toEqual(deny)
    expect(elevatorDenyFor('floor_02')).toBeNull()
    expect(POI_INSPECT.poi_elevator_door_f2).toContain('Floors 3–5')
    expect(POI_INSPECT.poi_elevator_door_f3).not.toContain('above your grade')
    expect(POI_INSPECT.poi_elevator_door_f4).not.toContain('above your grade')
  ...
  it('uses the same employee-badge deny when the panel is opened on Floor 2', () => {
