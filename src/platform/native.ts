...
/**
 * Night-lobby midtone / Title field floor (`--cc-bg`): the one boot color.
 * Every surface on the cold-launch path is this value — LaunchScreen
 * storyboard art, SplashScreen plugin, WKWebView background, `html/body/#root`,
 * `.boot-splash`, Stage card — so the native splash dissolves into the web
 * boot splash with no color step. Mirrored (not imported — the Capacitor CLI
 * loads that file outside Vite) in `capacitor.config.ts`, `index.html`, and
 * `public/manifest.webmanifest`; `boot-splash.test.ts` keeps them in step.
 */
export const BOOT_COLOR = '#12141a'
 ...
 * anything but `never`, double-inset (not fine). Night-lobby midtone for the
 * bar background covers the overlay-off case so a flip can't flash the
 * default `#000000`. Style.Dark is light glyphs on a dark bar.
 */
