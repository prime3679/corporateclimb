...
import { ALL_ITEM_IDS, ITEMS, PERKS } from '@/data'
...
/** Price after economy perks. */
export function shopPrice(basePrice: number, perks: PerkId[]): number {
  const mult = perks.reduce((m, id) => m * (PERKS[id]?.priceMult ?? 1), 1)
...
export function buyShopItem(run: RunState, stockIdx: number): RunState {
  ...
  const price = shopPrice(ITEMS[itemId].price, run.perks)
...
export function buyWellnessDay(run: RunState, effectiveMaxHp: number): RunState {
  const price = shopPrice(WELLNESS_DAY.price, run.perks)
